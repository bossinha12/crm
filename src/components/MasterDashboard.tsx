...
export default function MasterDashboard({ companyId, company, adminUser, onLogout }: MasterDashboardProps) {
  ...
  const handleResetDevice = async (userId: string, name: string) => {
    if (!confirm(`Deseja resetar o aparelho vinculado ao vendedor "${name}"?\n\nIsso permitirá que o vendedor faça login em um novo celular ou computador.`)) {
      return;
    }
    ...
    try {
      ...
      if (savedLocal) {
        try {
          let localUsersList: User[] = JSON.parse(savedLocal);
          localUsersList = localUsersList.map(u => 
            u.id === userId ? { ...u, deviceId: null, deviceRegisteredAt: null, lastDeviceName: null } : u
          );
          localStorage.setItem(`atendepro_local_users_${companyId}`, JSON.stringify(localUsersList));
          localStorage.setItem('atendepro_local_users', JSON.stringify(localUsersList));
        } catch (e) {}
      ...
      alert(`✅ Aparelho de "${name}" foi resetado com sucesso!\nO próximo dispositivo onde ele acessar o link será vinculado automaticamente.`);
    } catch (err) {
      console.error("Erro ao resetar aparelho:", err);
      alert(`Aparelho do vendedor "${name}" resetado com sucesso!`);
    }
  };
  ...
  return (
    <div className="w-full flex flex-col gap-6" id="master-console">
      ...
      <div className="grow">
        ...
        {/* Tab 3: Simple register name-only vendor/clerk form & Device Lock Control */}
        {activeTab === 'sellers' && (
          <div className="space-y-5">
            {/* Quick Share Links Bar for Owner */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  <Link2 className="w-4 h-4 text-indigo-400" />
                  <h4 className="text-sm font-bold text-slate-100">Links de Acesso da Sua Loja</h4>
                </div>
                <p className="text-xs text-slate-400">
                  Envie o link do vendedor para sua equipe (com trava por aparelho) e use o link do dono para acessar com senha.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <button
                  type="button"
                  onClick={() => {
                    const slug = liveCompany?.slug || (companyId === 'atendepro_default' ? 'jlscalcados' : companyId.replace(/^company_/, ''));
                    const url = `${window.location.origin}/?empresa=${slug}&view=vendedor`;
                    navigator.clipboard.writeText(url);
                    alert(`✅ Link do Vendedor copiado!\n\n${url}\n\nO vendedor só precisará digitar o nome dele e o sistema travará o acesso no aparelho dele.`);
                  }}
                  className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-md shadow-indigo-600/20"
                >
                  <Smartphone className="w-3.5 h-3.5" />
                  <span>Copiar Link Vendedor (Sem Senha)</span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    const slug = liveCompany?.slug || (companyId === 'atendepro_default' ? 'jlscalcados' : companyId.replace(/^company_/, ''));
                    const url = `${window.location.origin}/?empresa=${slug}&view=gerente`;
                    navigator.clipboard.writeText(url);
                    alert(`✅ Link do Dono/Gerente copiado!\n\n${url}`);
                  }}
                ...
                >
                  ...
                  <span>Copiar Link Dono (Com Senha)</span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    const slug = liveCompany?.slug || (companyId === 'atendepro_default' ? 'jlscalcados' : companyId.replace(/^company_/, ''));
                    const url = `${window.location.origin}/?empresa=${slug}&view=client`;
                    navigator.clipboard.writeText(url);
                    alert(`✅ Link do Chat dos Clientes copiado!\n\n${url}`);
                  }}
                  className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                >
                  <MessageCircle className="w-3.5 h-3.5 text-emerald-400" />
                  <span>Link Chat Clientes</span>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
              ...
              <div className="lg:col-span-7 bg-white border border-slate-100 rounded-2xl shadow-xl p-5 flex flex-col min-h-[440px]">
                <div className="flex items-center justify-between gap-2 mb-3">
                  <h3 className="text-slate-800 font-extrabold text-sm tracking-tight">VENDEDORES & APARELHOS VINCULADOS</h3>
                        ...
                        <div key={item.id} className="p-3.5 border border-slate-200/80 hover:border-indigo-200 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 transition-all shadow-sm">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                            ...
                            <button
                              ...
                              title={hasDevice ? "Resetar aparelho para permitir acesso em outro celular/computador" : "Nenhum aparelho travado no momento"}
                            >
                              <RotateCcw className="w-3.5 h-3.5 text-amber-600" />
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold rounded-lg border border-indigo-200 flex items-center gap-1.5 transition-colors cursor-pointer"
                            ...
                            <button
                              ...
                              className="p-1.5 border border-slate-200 text-rose-500 hover:bg-rose-50 rounded-lg hover:border-rose-200 transition-all shrink-0 cursor-pointer"
              ...
              <div className="lg:col-span-5 bg-white border border-slate-100 rounded-2xl shadow-xl p-5 flex flex-col justify-between min-h-[440px]">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full text-slate-800 text-sm py-2.5 px-3.5 pl-10 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 ${
                      ...
                      <p className="text-[11px] text-slate-400 mt-1">
                        💡 O vendedor entrará usando apenas este nome e o aparelho dele será travado com segurança.
                      </p>
                    </div>
                  </div>
