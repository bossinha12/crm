...
export default function ClientWidget({ companyId, companyName, companyLogo, onGoBack }: ClientWidgetProps) {
  ...
  return (
    <div className="max-w-2xl w-full bg-slate-50 rounded-2xl border border-slate-200 shadow-xl overflow-hidden h-[630px] flex flex-col transition-all duration-300">
      ...
      <div className="bg-gradient-to-r from-indigo-700 via-indigo-600 to-slate-900 text-white px-5 py-3.5 shrink-0 flex items-center justify-between shadow-md relative">
        <div className="flex items-center gap-3">
          ...
          <div className="relative shrink-0">
            ...
            {/* Online Status Dot */}
            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-indigo-700 flex items-center justify-center shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-sm tracking-tight text-white flex items-center gap-1.5">
                <span>{activeChat?.sellerName ? activeChat.sellerName : companyName}</span>
                {activeChat?.sellerName && (
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">
                    <ShieldCheck className="w-3 h-3 text-emerald-300" />
                    <span>Atendente Real</span>
                  </span>
                )}
              </h3>
              <span className="text-[10px] bg-slate-800 text-slate-200 border border-slate-700 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wider">
            ...
            <div className="text-xs text-indigo-100 flex items-center gap-1.5 mt-0.5">
              ...
              <span>
                {activeChat?.status === ChatStatus.NEW && 'Aguardando atendente iniciar...'}
                {activeChat?.status === ChatStatus.ACTIVE && `Você está conversando com ${activeChat.sellerName}`}
        ...
        <div>
          <button
            ...
            className="text-xs bg-black/25 hover:bg-black/40 px-3 py-1.5 rounded-xl text-white font-semibold flex items-center gap-1.5 transition-colors border border-white/15 cursor-pointer shadow-sm"
          >
      ...
      <div ref={clientMessagesContainerRef} className="grow overflow-y-auto p-5 space-y-4 bg-slate-50/50" id="messages-stream">
        ...
        {messages.map((m) => {
          ...
          return (
            <div
              key={m.id}
              className={`flex gap-2.5 ${isMe ? 'justify-end' : 'justify-start'} animate-fade-in items-end`}
            >
              {/* Seller Avatar Thumbnail next to seller message */}
              {!isMe && (
                <div className="w-8 h-8 rounded-full border border-slate-200 overflow-hidden shrink-0 bg-indigo-100 flex items-center justify-center mb-1 shadow-sm">
                  {sellerPhoto ? (
                    <img 
                      src={sellerPhoto} 
                      referrerPolicy="no-referrer" 
              ...
              <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[80%]`}>
                <div className="text-[10px] text-slate-400 font-bold px-1 mb-0.5 uppercase tracking-wide flex items-center gap-1">
                  <span>{isMe ? 'Você' : m.senderName}</span>
                  {!isMe && m.senderName !== 'Sistema' && (
                    <span className="text-[9px] text-emerald-600 bg-emerald-50 px-1 py-0.2 rounded font-semibold border border-emerald-100">
                      ✓ Atendente
                    </span>
                  )}
                </div>
                ...
                <div
                  className={`rounded-2xl text-sm leading-relaxed overflow-hidden shadow-sm ${
                    isMe
                      ? 'bg-indigo-600 text-white rounded-br-none shadow-indigo-600/10'
                      : 'bg-white text-slate-800 rounded-bl-none border border-slate-200/80 shadow-slate-200/40'
